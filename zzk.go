...
import (
	...
	"github.com/smilextay/dm/parser"
		...
		case parser.STRING:
			{
				...
				if len(lval.Value) > int(INT16_MAX) {
					...
					nsql.WriteString("?")
					optParamList = append(optParamList, newOptParameter(Dm_build_919.Dm_build_1135(lval.Value, serverEncoding, dc), VARCHAR, VARCHAR_PREC))
				}
			}
		case parser.HEX_INT:
